...
- 61 篇可选预设：48 篇中文（含 19 篇联网核实的公版全文／整章及 17 篇用户提供歌曲）、8 篇英文（含 2 篇用户提供歌曲）、5 篇代码。
...
源站 `/assets/` 的构建哈希资源返回 `public, max-age=31536000, immutable`；首页和其他静态文件返回 `public, max-age=0, s-maxage=60, must-revalidate`，浏览器每次验证，边缘节点可缓存 60 秒。`/api/session`、`/api/health` 和错误响应返回 `no-store`；`/ws` 是实时连接，不缓存。发布后首页最多延迟约 60 秒更新，建议刷新 EdgeOne 的 `/` 与 `/index.html` 缓存，避免旧首页引用已被新镜像替换的构建资源。

旧版首页返回 `no-cache`，按 [EdgeOne 默认缓存规则](https://edgeone.ai/document/54213) 不会缓存，因此首页持续 MISS 并不代表服务故障。控制台需遵循源站缓存头；不要对全站强制缓存，尤其不要缓存身份接口。若哈希资源也一直 MISS，检查 EdgeOne 规则引擎是否配置了不缓存、缓存键是否变化以及 Nginx 是否覆盖源站响应头。

验证时对**同一个资源 URL 连续发出普通 GET 请求**，检查 `Cache-Control`、`Age` 与 `EO-Cache-Status`；不要追加随机查询参数，也不要开启浏览器“停用缓存”。首次 MISS 正常，后续命中还受节点、缓存过期和冷文件淘汰影响；状态含义见 [EdgeOne 文档](https://edgeone.ai/document/57250)。本地测试只能确认源站响应头，实际命中率需要在已部署域名上验证。
...
用户提供的《康神开播了》《优势在我》《下课》保留已接入正文；《康神开播了》按要求将“明天在玩”修正为“明天再玩”。另从 `src/cn`、`src/en` 接入 19 篇歌曲，保留用户提供版本及歌手署名，处理规则见文章来源说明。原始素材目录保持现有忽略配置，运行和 CI 使用已入库正文及内容指纹，不依赖本地素材文件。
